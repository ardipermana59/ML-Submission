...
def solution_C3():
    ...
    train_datagen = ImageDataGenerator(rescale=1. / 255., rotation_range=0.1, width_shift_range=0.1,
    ...
    # YOUR CODE HERE
    train_generator = train_datagen.flow_from_directory(directory=train_dir, target_size=(150, 150), color_mode='rgb',
                                                        class_mode='binary', shuffle=True, batch_size=16)

    val_generator = val_datagen.flow_from_directory(directory=validation_dir, target_size=(150, 150), color_mode='rgb',
                                                    class_mode='binary', shuffle=True, batch_size=16)
    ...
    model = tf.keras.models.Sequential([  # YOUR CODE HERE, end with a Neuron Dense, activated by 'sigmoid'
        ...
        tf.keras.layers.MaxPooling2D(2, 2), tf.keras.layers.Conv2D(128, (3, 3), activation='relu'),
        tf.keras.layers.MaxPooling2D(2, 2), tf.keras.layers.Flatten(), tf.keras.layers.Dense(512, activation='relu'),
    ...
    model.fit(train_generator, epochs=100, steps_per_epoch=(2000 // 16), validation_data=val_generator,
              validation_steps=(1000 // 16), callbacks=[CALLBACK])
