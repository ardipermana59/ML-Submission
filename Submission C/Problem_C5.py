...
def solution_C5():
    ...
    df = pd.read_csv('household_power_consumption.csv', sep=',', infer_datetime_format=True, index_col='datetime',
                     header=0)
    ...
    train_set = windowed_dataset(x_train, batch_size=BATCH_SIZE, n_past=N_PAST, n_future=N_FUTURE,
                                 shift=SHIFT)  # YOUR CODE HERE
    valid_set = windowed_dataset(x_valid, batch_size=BATCH_SIZE, n_past=N_PAST, n_future=N_FUTURE,
                                 shift=SHIFT)  # YOUR CODE HERE
    ...
    model = tf.keras.models.Sequential(
        [  # Whatever your first layer is, the input shape will be (N_PAST = 24, N_FEATURES = 7)
            # YOUR CODE HERE
            tf.keras.layers.Input(shape=(N_PAST, N_FEATURES)), tf.keras.layers.Dense(units=128, activation='relu'),
            tf.keras.layers.Flatten(),  # Flatten the output
            tf.keras.layers.Dense(units=N_FUTURE * N_FEATURES), tf.keras.layers.Reshape((N_FUTURE, N_FEATURES))])
    ...
    model.fit(train_set, epochs=1000, validation_data=valid_set, callbacks=[callbacks])
